...
<head>
    ...
    <title>Edición reactivada — {{ $edition->event->name }}</title>
</head>
    ...
    <tr>
        <td>
            <table width="600" cellpadding="0" cellspacing="0"
                ...
                <tr>
                    <td style="background:linear-gradient(135deg,#0f172a 0%,#0f766e 100%); padding:36px 48px; text-align:center;">
                        <p style="color:#5eead4; margin:0 0 4px; font-size:11px; letter-spacing:3px; text-transform:uppercase; font-weight:600;">Edición reactivada</p>
                ...
                <tr>
                    <td style="padding:40px 48px 8px;">
                        ...
                        <p style="color:#64748b; font-size:15px; line-height:1.7; margin:0;">
                            La edición de <strong>{{ $edition->event->name }}</strong> prevista para el
                            {{ $edition->date->translatedFormat('d \d\e F \d\e Y') }} a las {{ $edition->date->format('H:i') }} h
                            en {{ $edition->location }}, que había sido cancelada, ha sido reactivada por un administrador
                            y vuelve a estar activa. Te avisamos porque figuras como gestor/a de esta edición.
                        </p>
                    </td>
                </tr>
